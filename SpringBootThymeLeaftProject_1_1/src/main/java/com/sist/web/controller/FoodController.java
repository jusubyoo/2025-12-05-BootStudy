 ...
 *                 => DI의 종류
 *                    1) setter DI : set 메소드를 이용해서 변수의 초기화
 *                       <bean id="" class=""
 *                        p:name="" (properties)
 *                       />
 ...
 *                              @Service : BI(통합)
 *                                         DAO 여러개 사용
 *                                         부가적인 부분 => 로그인 (세션에 저장), 중복체크
 *                              @Controller : 화면 UI => 화면 제어 / 출력 데이터 전송
 *                              @RestController : 파일 제어 X
 *                              				  데이터만 전송 가능
 *                                                ---------- 다른 언어와 연동
 *                                                => Vue/React/모바일/JSON
 *                              @ControllerAdvice : 모든 Controller 에 예외처리를 공통으로 사용
 *               = AOP : 횡단지향적 프로그램
 *                       => OOP에서 불가능한 프로그램을 제공
 *                       => OOP를 보완
 *                       => OOP에서는 자동 호출 불가능 => 메소드 만들어서 호출
 *                          --- 소스 중복이 있는 경우
 *                              | 메소드(클래스 한 곳) / 여러개의 클래스 사용시 클래스 이용
 *                                ---------------- 공통 모듈 생성
 *                       => AOP에는 자동 호출(Callback : 시스템에서 자동 호출되는 메소드)
 *                          --- 공통 사용시에 지정
 *                          1. 어떤 메소드에서 호출할지 => PointCut
 *                          2. 메소드의 어떤 영역에서 처리 => JoinPoint
 *                             JoinPoint
 *                              = @Before : try 진입 전
 *                                          public void disp()
 *                                          {
 ...
 *                                          	}
 *                                          }
 *                              = @After : finally 에서 호출 => 무조건 수행
 *                              = @After-Throwable : catch 에서 수행
 *                              = @After-Returning : 정상 수행 => return 된 상태
 *                              = 코드
 *                                ------- 1
 *                                  코드
 ...
 *                       4. DispatcherServlet
 *                                  |
 *                               Controller 중에 URI 를 갖고 있는 메소드 찾기
 ...
 *                            --- MyBatis / JPA(Hibernate) => dataSet 이용
 *                                | XML에서 주로 사용
 * 	ThymeLeaf
 *  => Vue 와 비슷하다 (태그 안에서 제어문)
 *     <div v-for="">
 *     <div th:each="vo:${list}"> => 자바에서 사용하는 데이터를 그대로 사용
 *  => 서버 사이드 렌더링 엔진 => HTML로 변환
 *  => 화면 UI => Front 단
 *  => 자바와 호환성이 뛰어나다
 *  => JSP 보다 현대적이고 구조적이다
 ...
 *  바로 실행 여부
 *  --------------------------------------------
 *  SpringMVC 통합         매우 우수       제한적
 *  --------------------------------------------
 *  템플릿 문법			   현대식			 구식
 *  --------------------------------------------
 *  속도						빠름 		 느림
 *  --------------------------------------------
 *  현재 트렌드           아주 많이 사용     사용 x
 *  --------------------------------------------
 *  
 *  입문(학원)
 *  --------
 ...
 *     = 예외처리
 *     = 기본 라이브러리
 *       java.util :  Collection(List, Set, Map) 
 *                    *** 어노테이션 / JSON 파싱 / XML 파싱
 *       java.io : FileReader, FileWriter, BufferedReader, BufferedWriter
 *       java.lang : String, Object, Wrapper(Integer, Double, Boolean)
 ...
 *   *DQL		  *DML         DDL        *TCL
 *   ----         ----        ----        ----
 *   SELECT      INSERT	      CREATE      COMMIT / ROLLBACK
 *               UPDATE       ALTER
 *               DELETE       DROP
 *               			  TRUNCATE
 *                            RENAME
...
public class FoodController {
	...
	public String main_main(Model model)
	{
		...
		return "main"; // prefix:.html => main.html
	}
	...
	@GetMapping("/food/detail_before") // VO 단위로 받을 때는 RequestBody
	public String food_detail_before(@RequestParam(name = "fno",required = false) int fno, 
			HttpServletResponse response, RedirectAttributes ra) // repsponse 는 Cookie 전송, 다운로드 할 때 필요하다
	// request = cookie 목록
	{
		...
		// redirect 에서 다른 파일로 값을 전송 : RedirectAttributes
		// forward : Model
	...
	public String food_detail(@RequestParam(name = "fno",required = false) int fno, Model model) // 데이터 전송
	{
		// 1. hit 증가
		// 2. 상세보기 데이터 읽기
